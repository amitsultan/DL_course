...
class ANN:
    ...
    def L_model_forward(self, X, parameters, use_batchnorm=False, dropout=None):
        """
        Description:
        ...
        for layer_index in range(1, self.n_layers):
            ...
            if use_batchnorm:
                A_prev = self.apply_batchnorm(A_prev)
    ...
    def L_layer_model(self, X, Y, layers_dims, learning_rate, num_iterations, batch_size, batchnorm=False, dropout=None):
        """
        Description:
        ...
        for epoch in range(num_iterations):
            if converged:
                print(f"Converged at epoch {epoch} out of {num_iterations} after {iters} iterations")
                self.print_overall_accuracy(params,X_validation=X_validation, Y_validation=Y_validation,
                                            X_train=X_train, Y_train=Y_train, batchnorm=batchnorm)
                break
            for index in range(0, X_train.shape[1], batch_size):
                ...
                z_output, cache = self.L_model_forward(batch_x, params, batchnorm, dropout)
                iters += 1
                if iters % 100 == 0:
                    acc = self.predict(X_validation, Y_validation, params, batchnorm)
                    acc_train = self.predict(batch_x, batch_y, params, batchnorm)
                    ...
                    print(f"{training_step} Training Step - Accuracy = {acc}, cost = {cost}, acc_train = {acc_train}")
                    training_step += 1
                grads = self.L_model_backward(z_output, batch_y, cache)
                params = self.update_parameters(params, grads, learning_rate)
        if not converged:
            self.print_overall_accuracy(params, batchnorm, X_validation=X_validation, Y_validation=Y_validation,
                                        X_train=X_train, Y_train=Y_train)
            print(f'took {iters} iterations and did not converge')
        return params, history

    def predict(self, X, Y, parameters, batchnorm=False):
        """
        Description:
        ...
        """
        y_preds, cache = self.L_model_forward(X, parameters, batchnorm)
        a = np.argmax(y_preds, axis=0)
        y = np.argmax(Y, axis=0)
        acc = np.sum(np.equal(y, a)) / len(y)
        return acc

    def print_overall_accuracy(self,params, batchnorm=False, **kwargs):
        """
        ...
        if 'X_train' in kwargs and 'Y_train' in kwargs:
            ...
            print(f"Training accuracy: {self.predict(X_train, Y_train, params, batchnorm)}")
        if 'X_validation' in kwargs and 'Y_validation' in kwargs:
            X_validation = kwargs['X_validation']
            Y_validation = kwargs['Y_validation']
            print(f"Validation accuracy: {self.predict(X_validation, Y_validation, params, batchnorm)}")
        if 'X_test' in kwargs and 'Y_test' in kwargs:
            X_test = kwargs['X_test']
            Y_test = kwargs['Y_test']
            print(f"Test accuracy: {self.predict(X_test, Y_test, params, batchnorm)}")
